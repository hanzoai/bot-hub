...
async function describeOwnerVisibleSkillState(
  ...
  if (skill.softDeletedAt) {
    return {
      status: 410,
      message: `Skill is hidden/deleted. Run "bothub undelete ${slug}" to restore it.`,
    }
  }
  ...
  if (skill.moderationStatus === 'hidden') {
    ...
    if (skill.moderationReason === 'quality.low') {
      return {
        status: 403,
        message:
          'Skill is hidden by quality checks. Update SKILL.md content or run "bothub undelete <slug>" after review.',
      }
    }
    return {
      status: 403,
