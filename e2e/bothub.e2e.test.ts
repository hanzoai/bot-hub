...
import { readGlobalConfig } from '../packages/bothub/src/config'
...
function mustGetToken() {
  const fromEnv = process.env.BOTHUB_E2E_TOKEN?.trim() || process.env.CLAWDHUB_E2E_TOKEN?.trim()
...
function getRegistry() {
  return (
    process.env.BOTHUB_REGISTRY?.trim() ||
...
function getSite() {
  return (
    process.env.BOTHUB_SITE?.trim() || process.env.CLAWDHUB_SITE?.trim() || 'https://hub.hanzo.bot'
  )
}

async function makeTempConfig(registry: string, token: string | null) {
  const dir = await mkdtemp(join(tmpdir(), 'bothub-e2e-'))
...
describe('bothub e2e', () => {
  it('prints CLI version via --cli-version', async () => {
    const result = spawnSync('bun', ['bothub', '--cli-version'], {
      cwd: process.cwd(),
      encoding: 'utf8',
    })
  ...
  it('cli search does not error on multi-result responses', async () => {
    ...
    try {
      const workdir = await mkdtemp(join(tmpdir(), 'bothub-e2e-workdir-'))
      const result = spawnSync(
        ...
        {
          cwd: process.cwd(),
          env: { ...process.env, BOTHUB_CONFIG_PATH: cfg.path, BOTHUB_DISABLE_TELEMETRY: '1' },
          encoding: 'utf8',
        },
      )
  ...
  it('assumes a logged-in user (whoami succeeds)', async () => {
    ...
    const token = mustGetToken() ?? (await readGlobalConfig())?.token ?? null
    if (!token) {
      throw new Error('Missing token. Set BOTHUB_E2E_TOKEN or run: bun bothub auth login')
    }
    ...
    try {
      ...
      const result = spawnSync(
        'bun',
        ['bothub', 'whoami', '--site', site, '--registry', registry],
        {
          cwd: process.cwd(),
          env: { ...process.env, BOTHUB_CONFIG_PATH: cfg.path, BOTHUB_DISABLE_TELEMETRY: '1' },
          encoding: 'utf8',
  ...
  it('sync dry-run finds skills from an explicit root', async () => {
    ...
    const cfg = await makeTempConfig(registry, token)
    const root = await mkdtemp(join(tmpdir(), 'bothub-e2e-sync-'))
    try {
  ...
  it('sync dry-run finds skills from clawdbot.json roots', async () => {
    ...
    const cfg = await makeTempConfig(registry, token)
    const root = await mkdtemp(join(tmpdir(), 'bothub-e2e-clawdbot-'))
    ...
    try {
      ...
      const result = spawnSync(
        'bun',
        ['bothub', 'sync', '--dry-run', '--all', '--site', site, '--registry', registry],
        {
          cwd: process.cwd(),
          env: {
            ...process.env,
            BOTHUB_CONFIG_PATH: cfg.path,
            BOTHUB_DISABLE_TELEMETRY: '1',
  ...
  it('publishes, deletes, and undeletes a skill (logged-in)', async () => {
    ...
    const cfg = await makeTempConfig(registry, token)
    const workdir = await mkdtemp(join(tmpdir(), 'bothub-e2e-publish-'))
    const installWorkdir = await mkdtemp(join(tmpdir(), 'bothub-e2e-install-'))
    ...
    try {
      ...
      const list = spawnSync(
        'bun',
        ['bothub', 'list', '--site', site, '--registry', registry, '--workdir', installWorkdir],
        {
          cwd: process.cwd(),
          env: { ...process.env, BOTHUB_CONFIG_PATH: cfg.path, BOTHUB_DISABLE_TELEMETRY: '1' },
          encoding: 'utf8',
  ...
  it('delete returns proper error for non-existent skill', async () => {
    const registry = process.env.CLAWDHUB_REGISTRY?.trim() || 'https://hub.hanzo.bot'
    const site = process.env.CLAWDHUB_SITE?.trim() || 'https://hub.hanzo.bot'
    const token = mustGetToken() ?? (await readGlobalConfig())?.token ?? null
    if (!token) {
      throw new Error('Missing token. Set CLAWDHUB_E2E_TOKEN or run: bun bothub auth login')
    }

    const cfg = await makeTempConfig(registry, token)
    const workdir = await mkdtemp(join(tmpdir(), 'bothub-e2e-delete-'))
