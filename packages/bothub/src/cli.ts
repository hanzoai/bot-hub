...
const program = new Command()
  .name('bothub')
  .description(
    `${styleTitle(`Bot Hub CLI ${getCliBuildLabel()}`)}\n${styleEnvBlock(
...
async function resolveGlobalOpts(): Promise<GlobalOpts> {
  ...
  const site = raw.site ?? process.env.BOTHUB_SITE ?? process.env.CLAWDHUB_SITE ?? DEFAULT_SITE
  const registrySource = raw.registry
    ? 'cli'
    : process.env.BOTHUB_REGISTRY || process.env.CLAWDHUB_REGISTRY
      ? 'env'
      : 'default'
  const registry =
    raw.registry ??
    process.env.BOTHUB_REGISTRY ??
...
async function resolveWorkdir(explicit?: string) {
  ...
  const envWorkdir = process.env.BOTHUB_WORKDIR?.trim() ?? process.env.CLAWDHUB_WORKDIR?.trim()
...
async function hasBothubMarker(workdir: string) {
  const lockfile = join(workdir, '.bothub', 'lock.json')
  if (await pathExists(lockfile)) return true
  const markerDir = join(workdir, '.bothub')
  if (await pathExists(markerDir)) return true
  const legacyLockfile = join(workdir, '.bothub', 'lock.json')
  if (await pathExists(legacyLockfile)) return true
  const legacyMarkerDir = join(workdir, '.bothub')
