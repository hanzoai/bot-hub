...
const mockGetRegistry = vi.fn(async () => 'https://hub.hanzo.bot')
vi.mock('../registry.js', () => ({
...
describe('cmdLogout', () => {
  it('removes token and logs a clear message', async () => {
    mockReadGlobalConfig.mockResolvedValueOnce({ registry: 'https://hub.hanzo.bot', token: 'tkn' })
