...
const testConfigPath = '/tmp/bothub-config-test/config.json'
...
beforeEach(() => {
  vi.stubEnv('BOTHUB_CONFIG_PATH', testConfigPath)
...
describe('writeGlobalConfig', () => {
  it('writes config with restricted modes', async () => {
    ...
    expect(mkdirMock).toHaveBeenCalledWith('/tmp/bothub-config-test', {
      recursive: true,
      mode: 0o700,
    })
