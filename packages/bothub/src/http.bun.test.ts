...
async function loadHttpModuleWithBunMocks(opts?: {
  ...
  const mkdtemp = vi.fn(async () => opts?.mkdtempValue ?? '/tmp/bothub-test')
...
describe('http bun runtime', () => {
  ...
  it('handles downloadZip bun path and cleans up temp dir', async () => {
    ...
    const { http, rm, readFile } = await loadHttpModuleWithBunMocks({
      spawnImpl: spawnSync,
      mkdtempValue: '/tmp/bothub-download-abc',
    ...
    expect(rm).toHaveBeenCalledWith('/tmp/bothub-download-abc', {
      recursive: true,
      force: true,
    })
  })
  ...
  it('posts multipart form via curl in bun path', async () => {
    ...
    const { http, writeFile, rm } = await loadHttpModuleWithBunMocks({
      spawnImpl: spawnSync,
      mkdtempValue: '/tmp/bothub-upload-abc',
    })
    ...
    expect(rm).toHaveBeenCalledWith('/tmp/bothub-upload-abc', { recursive: true, force: true })
    const [, args] = spawnSync.mock.calls[0] as [string, string[]]
    expect(args).toContain('-F')
    expect(args.some((arg) => arg.includes('name=demo'))).toBe(true)
    expect(args.some((arg) => arg.includes('file=@/tmp/bothub-upload-abc/demo.txt'))).toBe(true)
  })
})
