...
describe('skills', () => {
  it('extracts zip into directory and skips traversal', async () => {
    const parent = await mkdtemp(join(tmpdir(), 'bothub-zip-'))
  ...
  it('writes and reads lockfile', async () => {
    const workdir = await mkdtemp(join(tmpdir(), 'bothub-work-'))
  ...
  it('returns empty lockfile on invalid json', async () => {
    const workdir = await mkdtemp(join(tmpdir(), 'bothub-work-bad-'))
    await mkdir(join(workdir, '.bothub'), { recursive: true })
    await writeFile(join(workdir, '.bothub', 'lock.json'), '{', 'utf8')
    const read = await readLockfile(workdir)
    expect(read).toEqual({ version: 1, skills: {} })
  })

  it('returns empty lockfile on schema mismatch', async () => {
    const workdir = await mkdtemp(join(tmpdir(), 'bothub-work-schema-'))
    await mkdir(join(workdir, '.bothub'), { recursive: true })
    await writeFile(
      join(workdir, '.bothub', 'lock.json'),
  ...
  it('skips dotfiles and node_modules when listing text files', async () => {
    const workdir = await mkdtemp(join(tmpdir(), 'bothub-files-'))
  ...
  it('respects .gitignore and .bothubignore', async () => {
    const workdir = await mkdtemp(join(tmpdir(), 'bothub-ignore-'))
    await writeFile(join(workdir, '.gitignore'), 'ignored.md\n', 'utf8')
    await writeFile(join(workdir, '.bothubignore'), 'private.md\n', 'utf8')
    await writeFile(join(workdir, 'SKILL.md'), 'hi', 'utf8')
  ...
  it('falls back to text/plain for unknown text extensions', async () => {
    const workdir = await mkdtemp(join(tmpdir(), 'bothub-env-'))
    await writeFile(join(workdir, 'SKILL.md'), 'hi', 'utf8')
  ...
  it('returns null for invalid skill origin metadata', async () => {
    const workdir = await mkdtemp(join(tmpdir(), 'bothub-origin-'))
    expect(await readSkillOrigin(workdir)).toBeNull()

    await mkdir(join(workdir, '.bothub'), { recursive: true })
    await writeFile(
      join(workdir, '.bothub', 'origin.json'),
